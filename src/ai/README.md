...
### Steps
1. 阶段 A：依赖与数据契约（阻塞后续）
2. 新增运行依赖 pynput 和 markdown，定义统一消息结构（role、content、render_type、timestamp）与面板状态机（hidden、inputting、requesting、error）。
3. 阶段 B：全局输入监听（依赖 A）
4. 在启动链路初始化全局监听桥，统一捕获空格按下/抬起与鼠标中键点击；空格走 1000ms 长按判定并做去抖（按住只触发一次，抬起复位）；所有监听回调通过 Qt 信号切回主线程，禁止子线程直接改 UI。
5. 阶段 C：供应商/模型选择、配置与 API 调用（依赖 A，可与阶段 D 并行）
6. 建立 AI 配置服务并写入 QSettings（ai/provider、ai/api_key、ai/base_url、ai/model、ai/context_rounds）；首次打开面板按“先选供应商/模型，再输入 Key”流程执行；供应商提供预置 base_url 和默认模型；调用 OpenAI 兼容 chat/completions 接口，统一处理超时、鉴权失败、限流、服务异常与响应结构异常。
7. 阶段 D：回复格式适配（依赖 A，可与阶段 C 并行）
8. 新增响应格式化层：先尝试 JSON 解析并格式化展示；否则走 Markdown 转 HTML；转换失败回退纯文本，并做基础清洗（换行、非法 HTML 过滤）。
9. 阶段 E：侧边对话面板 UI（依赖 B/C/D）
10. 新增独立顶层 AI 面板（跟随桌宠定位），包含历史消息区与输入区；Enter 提交后禁用输入并显示请求中状态；收到回复后恢复输入并自动聚焦下一轮；用户与 AI 分开渲染为独立文本块；二次触发统一隐藏，再次触发展示。
11. 阶段 F：主窗口集成与互斥策略（依赖 E）
12. 在主窗口集中管理 toggle_ai_panel；面板显示时与短时气泡协同避免重叠；处理与菜单、拖拽、自定义缩放模式的互斥边界；退出程序时优雅释放监听线程。
13. 阶段 G：打包与回归（依赖 B-F）
14. 更新依赖与打包配置，验证源码运行和 EXE 运行一致性。
